...
describe('ChargePaymentsController', () => {
  ...
  it('createPayment should call service.create(chargeId, dto, file)', async () => {
    ...
    const res = await controller.createPayment('c1', dto, file);

    expect(service.create).toHaveBeenCalledWith('c1', dto, file);
  ...
  it('createPayment should pass undefined when no file', async () => {
    ...
    const res = await controller.createPayment('c1', dto, undefined);

    expect(service.create).toHaveBeenCalledWith('c1', dto, undefined);
    expect(res).toEqual({ id: 'p1' });
  });

  it('list should call service.list(chargeId)', async () => {
  ...
  it('findOne should call service.findOne(chargeId, paymentId)', async () => {
    service.findOne.mockResolvedValue({ id: 'p1' } as any);

    const res = await controller.findOne('c1', 'p1');

    expect(service.findOne).toHaveBeenCalledWith('c1', 'p1');
    expect(res).toEqual({ id: 'p1' });
  });

  it('downloadProof should call service.getProofDownloadUrl(chargeId, paymentId)', async () => {
    service.getProofDownloadUrl.mockResolvedValue({ url: 'signed' } as any);

    const res = await controller.downloadProof('c1', 'p1');

    expect(service.getProofDownloadUrl).toHaveBeenCalledWith('c1', 'p1');
    expect(res).toEqual({ url: 'signed' });
  });

  it('update should call service.update(chargeId, paymentId, dto, file)', async () => {
    service.update.mockResolvedValue({ id: 'p1' } as any);

    const dto: any = { amountPaid: 20 };
    const file = makeFile('b.png');

    const res = await controller.update('c1', 'p1', dto, file);

    expect(service.update).toHaveBeenCalledWith('c1', 'p1', dto, file);
    expect(res).toEqual({ id: 'p1' });
  });

  it('remove should call service.remove(chargeId, paymentId)', async () => {
    service.remove.mockResolvedValue({ ok: true } as any);

    const res = await controller.remove('c1', 'p1');
