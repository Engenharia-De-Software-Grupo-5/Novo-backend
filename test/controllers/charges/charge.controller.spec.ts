...
describe('ChargesController', () => {
  ...
  it('list should call service.list({ tenantId, propertyId, status })', async () => {
    service.list.mockResolvedValue([{ id: 'c1' }] as any);

    const res = await controller.list('t1', 'p1', 'OPEN' as any);
  ...
  it('findOne should call service.findOne(chargeId)', async () => {
  ...
  it('update should call service.update(chargeId, dto)', async () => {
    service.update.mockResolvedValue({ id: 'c1' } as any);

    const res = await controller.update('c1', { status: 'X' } as any);

    expect(service.update).toHaveBeenCalledWith('c1', { status: 'X' });
    expect(res).toEqual({ id: 'c1' });
  });

  it('cancel should call service.cancel(chargeId)', async () => {
    service.cancel.mockResolvedValue({ id: 'c1' } as any);

    const res = await controller.cancel('c1');

    expect(service.cancel).toHaveBeenCalledWith('c1');
    expect(res).toEqual({ id: 'c1' });
  });

  it('remove should call service.remove(chargeId)', async () => {
