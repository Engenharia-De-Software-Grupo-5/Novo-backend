...
describe('GenerateContractService', () => {
  ...
  it('should throw NotFoundException when contract not found', async () => {
    ...
    await expect(service.execute('c1', 'ct1')).rejects.toThrow(NotFoundException);
    await expect(service.execute('c1', 'ct1')).rejects.toThrow('Contrato não encontrado');

    expect(contractsRepo.getById).toHaveBeenCalledWith('c1', 'ct1');
  });
  ...
  it('should throw NotFoundException when template missing', async () => {
    ...
    await expect(service.execute('c1', 'ct1')).rejects.toThrow(NotFoundException);
    await expect(service.execute('c1', 'ct1')).rejects.toThrow('Template não encontrado');
  });

  it('should throw NotFoundException when condominium missing', async () => {
    const c = makeValidContract();
    c.property.condominium = null as any;
    contractsRepo.getById.mockResolvedValue(c);

    const service = makeService();

    await expect(service.execute('c1', 'ct1')).rejects.toThrow(NotFoundException);
    await expect(service.execute('c1', 'ct1')).rejects.toThrow('Condomínio não encontrado');
  });

  it('should throw NotFoundException when tenant missing', async () => {
    const c = makeValidContract();
    (c as any).tenant = null;
    contractsRepo.getById.mockResolvedValue(c);

    const service = makeService();

    await expect(service.execute('c1', 'ct1')).rejects.toThrow(NotFoundException);
    await expect(service.execute('c1', 'ct1')).rejects.toThrow('Locatário não encontrado');
  });

  it('should throw NotFoundException when address missing', async () => {
    const c = makeValidContract();
    c.property.condominium.address = null as any;
    contractsRepo.getById.mockResolvedValue(c);

    const service = makeService();

    await expect(service.execute('c1', 'ct1')).rejects.toThrow(NotFoundException);
    await expect(service.execute('c1', 'ct1')).rejects.toThrow('Endereço não encontrado');
  });

  it('should generate pdf, upload to minio, and return url', async () => {
    contractsRepo.getById.mockResolvedValue(makeValidContract());

    templateEngine.parse.mockReturnValue('MD');
    pdfGenerator.generate.mockResolvedValue(Buffer.from('PDF'));
    minio.uploadFileBuffer.mockResolvedValue({ fileName: 'contracts/ct1_123.pdf' });

    const service = makeService();

    const res = await service.execute('c1', 'ct1');

    expect(templateEngine.parse).toHaveBeenCalledWith(expect.any(String), expect.any(Object));
    expect(pdfGenerator.generate).toHaveBeenCalledWith('MD');

    expect(minio.uploadFileBuffer).toHaveBeenCalledWith(
      expect.any(Buffer),
      expect.stringMatching(/^contracts\/ct1_/),
      'application/pdf',
    );

    expect(res).toEqual({ url: 'contracts/ct1_123.pdf' });
  });
  ...
  it('should use manualContent when provided', async () => {
    ...
    await service.execute('c1', 'ct1', 'MANUAL {{x}}');

    expect(templateEngine.parse).toHaveBeenCalledWith('MANUAL {{x}}', expect.any(Object));
  });
});